...
| 13  | Dokumentacja wyprzedzała repozytorium: README obiecywało zrzut ekranu, którego jeszcze nie było, log powoływał się na wyniki przeglądu „niżej” przed ich dopisaniem, a zdanie „brak importów między pakietami” było nieprecyzyjne (test importuje `worker/src/prompt.ts`).                                                     | Przegląd kodu                                             | Zrzut dodany po pierwszej udanej analizie na żywo, sekcja przeglądu dopisana, zdanie o importach poprawione.                                                                                          |
...
do `messages.ts`; niedokładne twierdzenia w dokumentacji.

**Świadomie niezmienione:** zamiana KV na Durable Objects (idealny licznik nie jest wart dodatkowej
warstwy w demo – ograniczenie opisane w README); `strictTypeChecked` w ESLint (wymaga konfiguracji
type-aware, mały zysk przy `tsc --strict`); przypinanie akcji GitHub do SHA i `npm audit` w CI; przy
